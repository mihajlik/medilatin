...
\title{Target Language Independent Acoustic Modeling for Medieval Latin}

% TSD 2017: a short form should be given in case the title is too long for the running head
\titlerunning{Cross-Native-Language Medieval Latin LVCSR}
...
The second one is training a Unified Simplified Grapheme (USG) acoustic model that can deal with cross-native-language variations.
...
We propose a system that is suitable to recognize medieval Latin speech for all speakers from the Visegrad region.\footnote{Sadly, but for this paper, although it belongs to the Visegrad region, the Slovakian language is not included in our experiments, due to the lack of speech data for acoustic model training.}
